...
def _logical_to_physical_qsa_slots(
    ...
    # Invalid metadata is padding, not a request to silently redirect an
    # index to the last cache row.  Keep the explicit validity mask and use a
    # zero sentinel only for the masked gather itself.  In particular, do not
    # clamp a bad request/block to make an out-of-bounds mapping look valid.
    valid_requests = (requests >= 0) & (requests < block_table.shape[0])
    valid_positions = positions >= 0
    safe_positions = torch.where(
        valid_positions, positions, torch.zeros_like(positions)
    )
    logical_blocks = torch.div(
        safe_positions, block_size, rounding_mode="floor"
    )
    valid_blocks = logical_blocks < block_table.shape[1]
    valid = valid_requests & valid_positions & valid_blocks
    safe_requests = torch.where(
        valid_requests, requests, torch.zeros_like(requests)
    )
    safe_blocks = torch.where(
        valid_blocks, logical_blocks, torch.zeros_like(logical_blocks)
    )
...
def compressed_qsa_slot_mapping(
    ...
    nonnegative_positions = logical_positions >= 0
    safe_logical_positions = torch.where(
        nonnegative_positions,
        logical_positions,
        torch.zeros_like(logical_positions),
    )
    compressed_positions = torch.div(
        safe_logical_positions, compress_ratio, rounding_mode="floor"
    )
