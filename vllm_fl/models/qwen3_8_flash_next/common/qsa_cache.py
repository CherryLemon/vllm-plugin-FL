...
try:
    ...
    _VLLM_HAS_CIRCULAR_BUFFER_SPEC = True
except ImportError:
    # CircularBufferSpec was added after the vLLM 0.24 ABI.  Keep the
    # official cache/metadata source intact and provide only the type-level
    # compatibility needed by the old planner.  The worker copies the QSA
    # backend's ``indexes_kv_by_block_stride`` bit onto this spec, allowing
    # vLLM 0.24 to pad the physical page while the logical block stays at the
    # scheduler block size.  This fallback is deliberately a FullAttentionSpec
    # subclass: v0.24's registry can resolve it without patching vLLM core.
    @dataclass(frozen=True, kw_only=True)
    class CircularBufferSpec(FullAttentionSpec):
        pass

    _VLLM_HAS_CIRCULAR_BUFFER_SPEC = False
...
def _token_to_req_indices(
    ...
    """Return the request index for every scheduled token.

    ``token_to_req_indices`` is a convenience method introduced after the
    vLLM 0.24 ABI.  The official QSA metadata source uses it directly, so the
    old ABI needs this small equivalent producer for its torch fallback.  The
    CUDA/Triton producer does not call into ``CommonAttentionMetadata`` and is
    therefore unchanged.
    """

    token_to_req_fn = getattr(common_attn_metadata, "token_to_req_indices", None)
    if token_to_req_fn is not None:
        return token_to_req_fn(out)[:num_tokens]
    query_lens = (
    ...
    if num_mapped_tokens:
        mapped = torch.repeat_interleave(
            torch.arange(
                query_lens.shape[0], dtype=torch.int32, device=out.device
            ),
            query_lens,
...
def _build_qsa_metadata_torch(
    ...
    *,
    ...
    token_to_req = _token_to_req_indices(
...
class _QSAStateCache(nn.Module, AttentionLayerBase):
    ...
    def bind_kv_cache(self, kv_cache: torch.Tensor) -> None:
        """Bind the allocator-owned storage on the vLLM 0.24 ABI."""
...
class QSAKeyStateCache(_QSAStateCache):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        # The post-0.24 planner has a dedicated one-block circular manager.
        # vLLM 0.24 only knows the ordinary attention manager, so keep its
        # scheduler-visible block size at the global attention block size.
        # The official metadata/pre-indexer still uses this page as a ring;
        # the larger page is the ABI-only fallback and is padded by the worker
        # through ``indexes_kv_by_block_stride=True``.
        storage_block_size = (
            capacity if _VLLM_HAS_CIRCULAR_BUFFER_SPEC else self.cache_config.block_size
        )
