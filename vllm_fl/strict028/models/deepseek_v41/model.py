...
# Adapted from DeepSeek-V4.1-Flash/inference/model.py
# Source SHA256: 4e9ae23620edc8028ccc5d5fef552ab7fdc7dcd6f79608754fe9f67644056f65
# Original license: LICENSE.DeepSeek
import math
...
def linear(
    ...
    if weight.dtype in (torch.float4_e2m1fn_x2, torch.float8_e4m3fn):
        return lowp_linear(x, weight, weight.scale)
...
class ParallelEngramEmbedding(nn.Module):
    ...
    def __init__(self, num_embeddings: int, dim: int):
        ...
        self.scale = nn.Parameter(
            torch.empty(
                self.part_num_embeddings, dim // self.block_size, dtype=scale_dtype
            )
        )
...
class Indexer(torch.nn.Module):
    ...
    def __init__(self, args: ModelArgs, layer_id: int):
        ...
        self.wq_b = ColumnParallelLinear(
            self.q_lora_rank, self.n_heads * self.index_head_dim
        )
    ...
    def forward(
        ...
        # Each request owns a distinct cache page. Publish the owning layer's
        # cache even while its next compression group is incomplete.
        if self.owns_k:
...
class Attention(nn.Module):
    ...
    def _window_kv(self, x, freqs_cis, start_pos):
        ...
        if (
            start_pos == 0
        ):  # prefill: attend over this chunk, seeding the ring buffer for decode
            if seqlen <= win:
    ...
    def _compress_kv(self, x, qr, start_pos, offset):
        ...
        if latent is not None:
            ...
            self.compress_kv_cache[
                :bsz, start_pos // ratio : start_pos // ratio + latent.size(1)
            ] = latent
...
class DSparkBlock(Block):
    ...
    def forward(
        ...
        return super().forward(
            x, start_pos, pre_mix, None, main_x
        )  # drafts are text: no VL bias
...
class Transformer(nn.Module):
    ...
    def __init__(self, args: ModelArgs, tokenizer=None, *, enable_mtp=False):
        ...
        self.engram_hash = (
            NgramHashState(args, self.engram_layout, tokenizer)
        ...
        self.target_layer_ids = args.dspark_target_layer_ids if enable_mtp else ()
        if enable_mtp and args.dspark_block_size:
    ...
    def merge_image_embeddings(self, images, h: torch.Tensor):
        ...
        for i, sample in enumerate(images):
            for img in sample or ():
                ...
                embeds = self.encode_image(
                    img.patches.to(h.device), img.n_vit_h, img.n_vit_w
                )
    ...
    def forward(
        ...
        engram_hashes = (
            self.engram_hash(input_ids, start_pos, engram_mask)
